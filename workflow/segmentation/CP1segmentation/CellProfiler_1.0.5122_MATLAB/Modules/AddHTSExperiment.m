...
%textVAR10 = Lower and upper bounds of red intensity rescale, in the range [0,1]
%defaultVAR10 = 0,1
IntensityBoundRed = char(handles.Settings.VariableValues{CurrentModuleNum,10});

%textVAR11 = Lower and upper bounds of green intensity rescale, in the range [0,1]
%defaultVAR11 = 0,1
IntensityBoundGreen = char(handles.Settings.VariableValues{CurrentModuleNum,11});

%textVAR12 = Lower and upper bounds of blue intensity rescale, in the range [0,1]
%defaultVAR12 = 0,1
IntensityBoundBlue = char(handles.Settings.VariableValues{CurrentModuleNum,12});
...
if ~isfield(handles.Current, 'BatchInfo'),
    ...
    for i=3:size(dir_list, 1)
        ...
        if (dir_list(i).isdir)
            ...
            elseif WellTypeCode == 2
             ...
             handles = add_experiment(handles, WellTypeCode, VisualizationTypeCode, BaseFolder, ProjectName, dir_list(i).name, BaseExpFolder, ChannelCommon1, ChannelCommon2, ChannelCommon3, ImageResizeMethod, IntensityBoundRed, IntensityBoundGreen, IntensityBoundBlue);
    ...
    create_thumbnail(path, outPrefix, dir_list_r, dir_list_g, dir_list_b, imgprefix, ImageResizeMethod, IntensityBoundRed, IntensityBoundGreen, IntensityBoundBlue);
end;
...
function handles = add_experiment(handles, WellTypeCode, VisualizationTypeCode, BaseFolder, ProjectName, dir_list_name, BaseExpFolder, ChannelCommon1, ChannelCommon2, ChannelCommon3, ImageResizeMethod, IntensityBoundRed, IntensityBoundGreen, IntensityBoundBlue)
...
function out = create_thumbnail(prefix, outPrefix, r_list, g_list, b_list, imgprefix, ImageResizeMethod, IntensityBoundRed, IntensityBoundGreen, IntensityBoundBlue)
...
for i=1:list_length    
    if exist_r        
        rch(:, :, i) = double(imread([prefix r_list(i).name])');        
        IntensityBoundRed = str2num(IntensityBoundRed);
        rch(:, :, i) = imadjust(rch(:, :, i), [IntensityBoundRed(1) IntensityBoundRed(2)], [0 1]);
    else
        rch = 0;        
    end;

    if exist_g        
        gch(:, :, i) = double(imread([prefix g_list(i).name])');        
        IntensityBoundGreen = str2num(IntensityBoundGreen);
        gch(:, :, i) = imadjust(gch(:, :, i), [IntensityBoundGreen(1) IntensityBoundGreen(2)], [0 1]);        
    else        
        gch=0;        
    end;
    
    if exist_b        
        bch(:, :, i) = double(imread([prefix b_list(i).name])');        
        IntensityBoundBlue = str2num(IntensityBoundBlue);
        bch(:, :, i) = imadjust(bch(:, :, i), [IntensityBoundBlue(1) IntensityBoundBlue(2)], [0 1]);        
    else
        bch=0;        
    end;
end;
...
for i=1:list_length
    ...
    outimg = imresize(outimg, [1000 NaN], ImageResizeMethod);   
