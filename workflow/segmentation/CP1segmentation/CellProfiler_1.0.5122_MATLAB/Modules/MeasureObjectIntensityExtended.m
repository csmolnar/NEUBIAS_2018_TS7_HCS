function handles = MeasureObjectIntensityExtended(handles)
...
% the object.
% The function is extended by measuring the median intensity of the object
% and the std, mad, mean, and median of the top x percentage of pixles.
%
...
%
% $Revision: 5025 $
% Modified by Peter Horvath 2011. April
%
% Modified by Peter Horvath 2011. November
...
%textVAR08 = Give the ratio of the brightest pixels you want to use for the custom statistics (in percentile). For multiple values separate them with comma. Eg. 10,20,30
%defaultVAR08 = 20
percentiles = char(handles.Settings.VariableValues{CurrentModuleNum,8});
...
for i = 1:length(ObjectNameList)
    ...
    %%% parse the percentile string
    percentilesNum = str2num(percentiles);
    % remove less than 0 biger than 100
    percentilesNum(percentilesNum <= 0)   = [];
    percentilesNum(percentilesNum >= 100) = [];
    ...
    counter = length(BasicFeatures);
    
    for percI = 1:length(percentilesNum)
        BasicFeatures{counter} =  ['MeanIntensity_top_' num2str(percentilesNum(percI)) '_perc']; counter = counter + 1;
        BasicFeatures{counter} = ['MedianIntensity_top_' num2str(percentilesNum(percI)) '_perc']; counter = counter + 1;
        BasicFeatures{counter} = ['StdIntensity_top_' num2str(percentilesNum(percI)) '_perc']; counter = counter + 1;
        BasicFeatures{counter} = ['MadIntensity_top_' num2str(percentilesNum(percI)) '_perc']; counter = counter + 1;
    end;
    BasicFeatures
    ...
    if ObjectCount > 0
        ...
        for Object = 1:ObjectCount
            ...
            Basic(Object,12) = median(OrigImage(props(Object).PixelIdxList)); 
            
            %% sorted list
            sIntensities = sort(OrigImage(props(Object).PixelIdxList), 'descend');
     
            counter = 13;
            for percI = 1:length(percentilesNum)
                toTake = round(length(sIntensities) * percentilesNum(percI) / 100.0);
                Basic(Object, counter) = mean(sIntensities(1:toTake)); counter = counter + 1;
                Basic(Object, counter) = median(sIntensities(1:toTake)); counter = counter + 1;
                Basic(Object, counter) = std(sIntensities(1:toTake)); counter = counter + 1;
                Basic(Object, counter) = mad(sIntensities(1:toTake)); counter = counter + 1;
            end;            
        ...
        Basic(1,1:length(BasicFeatures)) = 0;
    end
    %%% Save measurements
