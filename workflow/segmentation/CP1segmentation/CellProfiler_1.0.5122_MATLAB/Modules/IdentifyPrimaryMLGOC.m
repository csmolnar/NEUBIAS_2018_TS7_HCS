    ...
    for ind=2:length(uniqueValues)
        ...
        if numh>numw
            ...
            if extraLabelNum>0
                ...
                try
                    objLayer(y0:y0+ho-1,x0+(numw-1)*2*radius:x0+wo-1) = objLayer(y0:y0+ho-1, x0+(numw-1)*2*radius:x0+wo-1)*(maxNum);
                catch
                    fprintf('(%d:%d,%d:%d)\n',y0,y0+ho-1,x0+(numw-1)*2*radius,x0+wo-1);
...
for ooiInd = 1:length(OverlappingObjectIndexes)
    ooi = OverlappingObjectIndexes(ooiInd);
    ObjectI = SegmentedLayers(:,:,OOL(ooiInd)) == ooi;
    % TODO: Centroid is already calculated!!!
    ...
    if ~ismember(ooi, RemovedIdxList)
        ...
        for oojInd = ooiInd:length(OverlappingObjectIndexes)
            ...
            if ~ismember(ooj, RemovedIdxList) && ooi~=ooj
                if sqrt((OOCentroids(ooiInd,1)-OOCentroids(oojInd,1))^2+(OOCentroids(ooiInd,2)-OOCentroids(oojInd,2))^2)<Radius*10
